...
class Denlac:
	...
	'''
	def agglomerative_clustering_fast(self, partitions, final_no_clusters, cluster_distance):
		#calculate distance matrix
		distanceMatrix = []

		for part1Id in partitions:
			matrixRow = []
			for part2Id in partitions:
				dist = 0
				if (part1Id != part2Id):
					if(cluster_distance==1):
						dist = self.calculate_centroid(partitions[part1Id], partitions[part2Id])					
					elif(cluster_distance==2):
						dist = self.calculate_average_pairwise(partitions[part1Id], partitions[part2Id])
					elif(cluster_distance==3):
						dist = self.calculate_smallest_pairwise(partitions[part1Id], partitions[part2Id])
				matrixRow.append(dist)
			distanceMatrix.append(matrixRow)

		#transform distance matrix https://stackoverflow.com/questions/18952587/use-distance-matrix-in-scipy-cluster-hierarchy-linkage
		'''distArray = ssd.squareform(distanceMatrix)

		print(np.shape(distanceMatrix))
		print(np.shape(distArray))
		print(distArray)''' 

		#apply agglomerative clustering on the partitions
		distanceMethod = 'single'
		if(cluster_distance==1):
		...
		'''Z = linkage(distArray, distanceMethod)
		clusters = fcluster(Z, final_no_clusters, criterion='maxclust')'''
		model = AgglomerativeClustering(affinity='precomputed', n_clusters=2, linkage=distanceMethod).fit(distanceMatrix)
		clusters = model.labels_

		print("Clusters len " + str(len(clusters)))

		#return data in the same format as agglomerative_clustering2

		cluster_points = collections.defaultdict(list)
		intermediary_centroids = []

		partitions_after_aggregation = collections.defaultdict(list)

		for clusterId in range(len(clusters)):
			cluster = clusters[clusterId]
			for point in partitions[clusterId]:
				partitions_after_aggregation[cluster].append(point)

		for partition_id in partitions_after_aggregation:
			partition_after_aggregation = partitions_after_aggregation[partition_id]
			partition_centroid = self.centroid(partition_after_aggregation)
			cluster_points[partition_centroid] = partition_after_aggregation
			intermediary_centroids.append(partition_centroid)
	...
	def calculate_average_pairwise(self, cluster1, cluster2):
		...
		return round(average_pairwise, 6)
	...
	'''
	returns the final_partitions as a dictionary with ids as keys and the objects as lists
	'''
	...
	def cluster_dataset(self, dataset_xy, dataset_xy_validate, each_dimension_values, clase_points):
		...
		duration = end - start
		print('Varianta veche dureaza: '+str(duration))

		'''start = time.time()
		intermediary_centroids, cluster_points = self.agglomerative_clustering_fast(final_partitions, self.no_clusters, self.cluster_distance)
		end = time.time()
		duration = end - start
		print('Varianta noua dureaza: '+str(duration))'''
