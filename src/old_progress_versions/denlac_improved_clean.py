...
class Denlac:
	...
	def __init__(self, no_clusters, no_bins, expand_factor, cluster_distance):
	...
	def agglomerativeClustering(self, partitions, final_no_clusters, cluster_distance):
		'''
			...
			for q in range(len(intermediary_centroids)):
				for p in range(q+1, len(intermediary_centroids)):
					...
					if(centroid_q!=centroid_p):
						# calculateSmallestPairwise pentru jain si spiral
						if(cluster_distance==1):
							dist = self.calculateCentroid(cluster_points[tuple(idx_dict_q)], cluster_points[tuple(idx_dict_p)])					
						elif(cluster_distance==2):
							dist = self.calculateAveragePairwise(cluster_points[tuple(idx_dict_q)], cluster_points[tuple(idx_dict_p)])
						elif(cluster_distance==3):
							dist = self.calculateSmallestPairwise(cluster_points[tuple(idx_dict_q)], cluster_points[tuple(idx_dict_p)])
						elif(cluster_distance==4):
							dist = self.calculateWeightedAveragePairwise(cluster_points[tuple(idx_dict_q)], cluster_points[tuple(idx_dict_p)])
						else:
							dist = self.calculateCentroid(cluster_points[tuple(idx_dict_q)], cluster_points[tuple(idx_dict_p)])
	...
	def calculateWeightedAveragePairwise(self, cluster1, cluster2):
		...
		for pixel1 in cluster1:
			for pixel2 in cluster2:
				distBetween = self.distanceFunction(pixel1, pixel2)
	...
	def calculateCentroid(self, cluster1, cluster2):
		...
		dist = self.distanceFunction(centroid1, centroid2)
		...
		for k in partition_dict:
			...
			for pixel_id in range(len(self.pixels_partition)):
				...
				if(self.pixels_partition[pixel_id][self.no_dims]==-1):
					...
					neigh_ids = self.getClosestKNeighbors(pixel, pixel_id)
	...
	def fitPredict(self, X, y, each_dimension_values, clase_points):
		...
		start = time.time()
		intermediary_centroids, cluster_points = self.agglomerativeClustering(final_partitions, self.no_clusters, self.cluster_distance) #paramateri: partitiile rezultate, numarul de clustere
		end = time.time()
		duration = end - start
		...
		for noise_point in noise:
			...
			for centroid in intermediary_centroids:
				for pixel in cluster_points[centroid]:
					dist = self.distanceFunction(noise_point, pixel)
					if(dist < minDist):
						minDist = dist
						closest_centroid = centroid
			cluster_points[closest_centroid].append(noise_point)

		self.evaluateCluster(clase_points, cluster_points)
		print("Evaluation")
		print("==============================")
	...
	def plotClusters(self, cluster_points, set_de_date, color_list):
...
if __name__ == "__main__":
	...
	cluster_distance = int(sys.argv[4])
	...
	denlacInstance = Denlac(no_clusters, no_bins, expand_factor, cluster_distance)
